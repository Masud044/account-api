...
import { getExpenseTotal, getExpenseBreakdown, getExpenseByAccount, getExpenseBreakdownSplit } from "./dashboard_expense.service.js";
...
// ─── Expense Breakdown — Project vs Other (date/description shoho) ─────
export async function getExpenseDetailsSplit(req, res) {
  try {
    const { month: rawMonth, year: rawYear, date: rawDate } = req.query;

    if (rawDate && rawDate !== "") {
      const data = await getExpenseBreakdownSplit({ date: rawDate });
      return res.status(200).json({ success: true, message: "Expense split breakdown fetched.", data });
    }
    ...
    const data = await getExpenseBreakdownSplit({ month, year });
    return res.status(200).json({ success: true, message: "Expense split breakdown fetched.", data });
  } catch (err) {
    console.error("[dashboard_expense] getExpenseDetailsSplit error:", err.message);
    return res.status(500).json({ success: false, message: "Internal server error." });
  }
}