...
export async function readSupplier(id) {
  ...
  try {
    const result = await connection.execute(
      "SELECT * FROM SUPPLIER_INFO WHERE SUPPLIER_ID = :supplier_id",
...
export async function readAllSuppliers() {
  ...
  try {
    const result = await connection.execute(
      "SELECT * FROM SUPPLIER_INFO ORDER BY SUPPLIER_ID DESC",
      {},
      { outFormat: oracledb.OUT_FORMAT_OBJECT }
    );
    return result.rows || [];
...
export async function updateSupplier(data) {
  ...
  try {
    ...
    const sql = `UPDATE SUPPLIER_INFO SET ${setClauses.join(", ")} WHERE SUPPLIER_ID = :supplier_id`;
