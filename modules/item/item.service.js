...
export const updateItem = async (itemId, data) => {
  ...
  try {
    ...
    `;
    const result = await conn.execute(sql, { itemId, ...data }, { autoCommit: true , outFormat: oracledb.OUT_FORMAT_OBJECT });
    return { rowsAffected: result.rowsAffected };
  } finally {
    await conn.close();
  }
...
export const getAllItems = async ({ page = 1, limit = 20 } = {}) => {
  ...
  try {
    ...
    `;
    const result = await conn.execute(sql, { offset, end: offset + limit }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
    return result.rows;
  } finally {
    await conn.close();
  }
...
export const getItemById = async (itemId) => {
  ...
  try {
    const result = await conn.execute(
      ...
      { itemId }, { outFormat: oracledb.OUT_FORMAT_OBJECT }
    );
