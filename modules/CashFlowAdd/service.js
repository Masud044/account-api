...
export async function insertCashFlow(payload) {
  return withConnection(async (connection) => {
    try {
      ...
      await connection.execute(
        `INSERT INTO glmaster (trans_date, voucher_type, description, voucherno, posted, GL_ENTRY_DATE, supporting, entry_by)
         VALUES (TO_DATE(:trans_date, 'YYYY-MM-DD'), 4, :receive_desc, :voucherno, 0, TO_DATE(:gl_date, 'YYYY-MM-DD'), :supporting, :entry_by)`,
        {
          trans_date: payload.trans_date,
          receive_desc: payload.receive_desc,
          voucherno: voucher,
          gl_date: payload.GL_ENTRY_DATE || payload.trans_date,
          supporting: payload.supporting,
          entry_by: payload.entry_by ?? null,
        },
        { autoCommit: false }
      );
      const idRow = await connection.execute("SELECT MAX(id) ID FROM glmaster", {}, { outFormat: oracledb.OUT_FORMAT_OBJECT });
